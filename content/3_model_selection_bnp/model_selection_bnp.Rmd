...
Two quantities are used:
  ...
  1) Pointwise log predictive density in-sample (lppd): $\sum_{i=1}^n \log \left(\frac{1}{M} \sum_{j=1}^M p(y_i | \theta^{(j)}) \right)$
...
$$ WAIC = -2 \mbox{lppd} + 2 p_{\mbox{WAIC}} $$

WAIC uses the full posterior, so does not rely on the plug-in predictive density as in DIC.

# Using WAIC

We'll use NIMBLE's default WAIC, which considers prediction of new observations (more later). 

```{r, waic}
outputFull <- nimbleMCMC(code = codeParam, data = data, inits = inits, constants = constants,
...
# WAIC can be tricky
 ...
 - There are different variations on WAIC depending on what predictive distribution is of interest:
    ...
    - New observations (conditional WAIC, default in NIMBLE)
         - For example, predicting observations for existing studies
    - New groups (often represented as a new random effect) (marginal WAIC)
         - For example, predicting new observations for new studies
...
modelFull <- nimbleModel(code = codeParam, data = data, inits = inits,
                      constants = constants)
mcmcFull <- buildMCMC(modelFull, enableWAIC = TRUE, controlWAIC = list(dataGroups = groups))
...
modelReduced <- nimbleModel(code = codeParamReduced, data = data, inits = inits,
                      constants = constants)
mcmcReduced <- buildMCMC(modelReduced, enableWAIC = TRUE, controlWAIC = list(dataGroups = groups))
...
modelFull <- nimbleModel(code = codeParam, data = data, inits = inits,
                      constants = constants)
mcmcFull <- buildMCMC(modelFull, enableWAIC = TRUE, controlWAIC = list(marginalizeNodes = 'gamma'))
...
modelReduced <- nimbleModel(code = codeParamReduced, data = data, inits = inits,
                      constants = constants)
mcmcReduced <- buildMCMC(modelReduced, enableWAIC = TRUE, controlWAIC = list(marginalizeNodes = 'gamma'))
...
To be honest, I'm not sure why the marginal WAIC doesn't more strongly favor the full model. One would think
...
Instead, we use the DP combined with a standard mixture modeling approach, such as a mixture of normal distributions. The CRP clusters items (in our case Avandia study random effects) to mixture components.
...
codeBNP <- nimbleCode({
  ...
  # hyperparameters
  alpha ~ dgamma(1, 1)      # 'alpha' in the CRP discussion
  theta ~ dflat()          # effect of Avandia
})
```
...
samplesBNP <- nimbleMCMC(code = codeBNP, data = data, inits = inits,
                         constants = constants,
                         monitors = c("theta", "gamma", "alpha", "xi"),
                         thin = 10, niter = 11000, nburnin = 1000, nchains = 1)
...
# BNP: more details
 ...
 - We have a [paper on using BNP in an ecological context](https://link.springer.com/article/10.1007/s10651-021-00489-1)
