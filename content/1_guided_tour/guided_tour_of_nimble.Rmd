...
subtitle: "NIMBLE ISEC 2022 (remote) workshop"
...
- Five modules, each 35-50 minutes with 5-10 minutes break = 45-55 minutes total.
- 1. Overview of `nimble`.
- 2. Extending models and methods using `nimbleFunction`s for efficiency and generality.
- 3. Model-selection with reversible jump (RJMCMC) and WAIC.  Bayesian non-parametric (BNP) distributions.
- 4. `nimbleEcology` distributions for capture-recapture, occupancy, dynamic occupancy, N-mixture, and Hidden Markove models (multi-state / multi-event capture-recapture).
- 5. Case study: Speeding up spatial capture-recapture (`nimbleSCR`).

We don't have time for spatial models (e.g. conditional auto-regressive models).  We'll point you to a recent workshop module on those.
...
- A library of other methods.
    ...
    - [Sequential Monte Carlo (particle filtering)](http://statistik-jstat.uibk.ac.at/article/view/v100i03)
    - Particle MCMC
    - Monte Carlo Expectation Maximization (maximum likelihood)
    - Reversible jump MCMC for variable selection. WAIC for model selection.
...
- NIMBLE is introducing support for automatic derivatives (AD) for any part of model.
- AD enables methods like Hamiltonian Monte Carlo (HMC, in package `nimbleHMC`) and Laplace approximation (in `nimble`).
- This is a complicated feature so we are entering a "beta release" stage for anyone who wants to be an early adopter and tester.
- See upcoming ISEC talks by Daniel Turek (HMC) and Wei Zhang (Laplace approximation).
- Beta release information and instructions are at [https://r-nimble.org/ad-beta](https://r-nimble.org/ad-beta).
- We will not cover these new features today.
...
* Modified from [Gimenez et al. 2021 online capture-recapture workshop](https://oliviergimenez.github.io/blog/hmmcrworkshop/)
